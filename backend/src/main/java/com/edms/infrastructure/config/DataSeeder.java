...
public class DataSeeder implements CommandLineRunner {
        ...
        public void run(String... args) throws Exception {
                ...
                // 1. Departments
                DepartmentEntity eng = departmentRepository.save(DepartmentEntity.builder().id("dpt1").code("ENG")
                                .name("Engineering").createdAt(Instant.now()).build());
                DepartmentEntity hr = departmentRepository.save(DepartmentEntity.builder().id("dpt2").code("HR")
                                .name("HR").createdAt(Instant.now()).build());
                DepartmentEntity mgmt = departmentRepository.save(DepartmentEntity.builder().id("dpt3").code("MGMT")
                                .name("Management").createdAt(Instant.now()).build());
                ...
                UserEntity owner = userRepository.save(UserEntity.builder()
                                ...
                                .role(UserRole.OWNER).department("Engineering").departmentId("dpt1").build());
                ...
                UserEntity editor = userRepository.save(UserEntity.builder()
                                ...
                                .role(UserRole.EDITOR).department("Engineering").departmentId("dpt1").build());
                ...
                UserEntity manager = userRepository.save(UserEntity.builder()
                                ...
                                .role(UserRole.MANAGER).department("Management").departmentId("dpt3").build());
                ...
                UserEntity viewer = userRepository.save(UserEntity.builder()
                                ...
                                .role(UserRole.VIEWER).department("HR").departmentId("dpt2").build());

                UserEntity admin = userRepository.save(UserEntity.builder()
                                .id("u5").email("admin@edms.vn").password(encodedPass).name("System Admin")
                                .role(UserRole.ADMIN).department("Management").departmentId("dpt3").build());

                // 3. Folders
                FolderEntity f1 = folderRepository.save(FolderEntity.builder()
                                .id("f1").name("Technical Contracts").department("Engineering").departmentId("dpt1")
                                .ownerId("u1").createdAt(Instant.now()).updatedAt(Instant.now()).build());

                FolderEntity f2 = folderRepository.save(FolderEntity.builder()
                                .id("f2").name("HR Policies").department("HR").departmentId("dpt2").ownerId("u4")
                                .createdAt(Instant.now()).updatedAt(Instant.now()).build());

                // 4. Documents & Versions
                DocumentEntity doc1 = documentRepository.save(DocumentEntity.builder()
                                .id("d1").title("Software Architecture Specification")
                                .type("Architecture Specification")
                                .status(DocumentStatus.APPROVED).ownerId("u1").folderId("f1").departmentId("dpt1")
                                .content("{\"architect\":\"Clean Architecture\"}")
                                .currentVersionId("v1").createdAt(Instant.now().minus(2, ChronoUnit.DAYS))
                                .updatedAt(Instant.now()).build());

                versionRepository.save(DocumentVersionEntity.builder()
                                .id("v1").documentId("d1").versionNumber(1)
                                .content("{\"architect\":\"Clean Architecture\"}").createdBy("u1")
                ...
                DocumentEntity doc2 = documentRepository.save(DocumentEntity.builder()
                                ...
                                .status(DocumentStatus.PENDING).ownerId("u2").folderId("f1").departmentId("dpt1")
                                .content("{\"total\":50000}")
                                .currentVersionId("v2").createdAt(Instant.now().minus(1, ChronoUnit.DAYS))
                                .updatedAt(Instant.now()).build());

                versionRepository.save(DocumentVersionEntity.builder()
                                .id("v2").documentId("d2").versionNumber(1).content("{\"total\":50000}").createdBy("u2")
                                .createdAt(Instant.now()).build());

                // 5. Permissions
                permissionRepository.save(PermissionEntity.builder().id("p1").documentId("d1").userId("u1")
                                .role(PermissionRole.OWNER).createdAt(Instant.now()).build());
                permissionRepository.save(PermissionEntity.builder().id("p2").documentId("d1").userId("u2")
                                .role(PermissionRole.EDITOR).createdAt(Instant.now()).build());
                permissionRepository.save(PermissionEntity.builder().id("p3").documentId("d1").userId("u4")
                                .role(PermissionRole.VIEWER).createdAt(Instant.now()).build());
                ...
                documentTagRepository.save(DocumentTagEntity.builder().id("dt1").documentId("d1").tagId("t1")
                                .createdAt(Instant.now()).build());
                documentTagRepository.save(DocumentTagEntity.builder().id("dt2").documentId("d1").tagId("t2")
                                .createdAt(Instant.now()).build());
                ...
                approvalHistoryRepository.save(ApprovalHistoryEntity.builder()
                                ...
                                .fromStatus(DocumentStatus.DRAFT).toStatus(DocumentStatus.PENDING).performedBy("u1")
                                .timestamp(Instant.now().minus(1, ChronoUnit.DAYS)).build());

                approvalHistoryRepository.save(ApprovalHistoryEntity.builder()
                                .id(UUID.randomUUID().toString()).documentId("d1").action(ApprovalAction.APPROVE)
                                .fromStatus(DocumentStatus.PENDING).toStatus(DocumentStatus.APPROVED).performedBy("u3")
                ...
                shareRepository.save(ShareEntity.builder()
                                ...
                                .token("share-token-123456").expiresAt(Instant.now().plus(7, ChronoUnit.DAYS))
                                .createdAt(Instant.now()).build());

                // 10. Audit Logs
                auditLogRepository.save(AuditLogEntity.builder()
                                .id("al1").documentId("d1").action(AuditAction.UPLOAD).performedBy("u1")
                                .details("Uploaded initial document").timestamp(Instant.now().minus(2, ChronoUnit.DAYS))
                                .build());
